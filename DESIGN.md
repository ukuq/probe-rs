...
| 项 | 约束 |
|---|---|
| `intervals` | 当前 Reporter 的六项采集需求，均 >= 1；collect 参与机器级最大公约数聚合，其他 worker 周期取最小值 |
...
- `last_seen` 记录网卡最后出现时间，用于回收长期消失的默认排除虚拟接口；旧账本没有该字段时不会被立即回收。
...
| 保留 | 32 天明细 + 永久归档基数 | 保留期必须严格大于最长账期（31 天），否则 reset_day 28-31 的账期首日明细会在账期内被归档、月流量永久少计；月账期可重算，实体网卡的 `reset_day=0` 不会因明细裁剪丢失累计；默认排除且已消失 32 天的临时虚拟接口会整项回收 |
...
2s × 31 天 ≈ 134 万条/网卡，全放内存约 30MB/网卡。实现保留最近 24 小时的 2 秒明细，更老的条目按小时合并（小时内 delta 求和），把每网卡长期记录压到约 24 小时细粒度 + 31 天小时桶。默认过滤的容器/VPN/桥接虚拟接口若连续 32 天未出现，会同时删除明细、归档基数和计数器基线，避免短命接口名称永久堆积；实体接口不做该回收。已有账本若解析失败，启动会明确报错并保留原文件，不会静默清零。
